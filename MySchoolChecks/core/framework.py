...
def _ask_send_options_gui(test_only=False):
    ...
    sw = win.winfo_screenwidth()
    sh = win.winfo_screenheight()
    win.geometry(f'360x240+{sw//2-180}+{sh//2-120}')
...
def run_check(check_module, config):
    ...
    # Φάκελος αποτελεσμάτων — ενιαίος φάκελος results_YYYYMMDD με υποφάκελο ανά έλεγχο
    ...
    out_dir = os.path.normpath(os.path.join(
        ...
        '..', f'results_{today.strftime("%Y%m%d")}', subfolder
    ))
